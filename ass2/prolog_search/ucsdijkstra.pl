...
% ucsdijkstra(Generated, Expanded, Solution, L, M, N)
%
...
% Expanded nodes are moved to another list (G is discarded)
...
% Insert newly created legs into the Generated list (in increasing order of G).
...
% and remove any existing leg with the same node
insert_one_leg([Leg1|Generated], Leg, [Leg|Generated1]) :-
    Leg  = [Node, _Pred, G ],
    Leg1 = [_Node1,_Pred1,G1],
    G < G1, !,
    remove_leg_with_node([Leg1|Generated], Node, Generated1).
...
% remove_leg_with_node(Legs, Node, Legs1).
% If list already contains a leg with this Node, remove it.

% base case: empty list remains empty
remove_leg_with_node([],_Node, []).

% If the first leg matches Node, remove it.
remove_leg_with_node([[Node|_]|Legs], Node, Legs) :- ! .

% Otherwise, apply recursively to rest of list.
remove_leg_with_node([Leg|Legs], Node, [Leg|Legs1]) :-
    remove_leg_with_node(Legs, Node, Legs1).
